...
<script>
  // Equalizador de alturas para las cards dentro del Swiper
  function equalizeSwiperCards(selector){
    const cards = document.querySelectorAll(selector + ' .swiper-slide .card');
  ...
  // Ejecutar cuando todas las imágenes carguen
  function whenImagesReady(cb){
    ...
    imgs.forEach(img=>{
      if(img.complete) { if(--pending===0) cb(); }
        ...
        img.addEventListener('load', ()=>{ if(--pending===0) cb(); });
        img.addEventListener('error', ()=>{ if(--pending===0) cb(); });
      }
    });
  }
  ...
  function initializeSwiperRANDOMID() {
      const swiper = new Swiper(".mySwiper-RANDOMID", {
          ...
          on: {
            init: () => equalizeSwiperCards('.mySwiper-RANDOMID'),
            resize: () => equalizeSwiperCards('.mySwiper-RANDOMID'),
          }
      });

      // Igualar una vez estén las imágenes listas
      whenImagesReady(()=> equalizeSwiperCards('.mySwiper-RANDOMID'));
      // Igualar al redimensionar ventana
      window.addEventListener('resize', ()=> equalizeSwiperCards('.mySwiper-RANDOMID'));
  }
</script>
@endsection
    ...
    <section>
        <div id="carouselExample" class="carousel slide" data-bs-ride="carousel">
            <div class="carousel-inner first ">
                <div class="carousel-item active" style="background:url({{ asset('/img/img6.jpg') }}) center / cover no-repeat;"></div>
                <div class="carousel-item" style="background:url('{{ asset('/img/viña.jpeg') }}') center / cover no-repeat;"></div>
                <div class="carousel-item" style="background:url('{{ asset('/img/viña2.jpeg') }}') center / cover no-repeat;"></div>
            </div>
            ...
            <div class="swiper mySwiper-RANDOMID">
                <div class="swiper-wrapper">
                    ...
                    <div class="swiper-slide">
                        <div class="card shadow mx-auto h-100 actividad-card" style="border-radius:20px;">
                            <img class="card-img-top img-fluid" src="{{ asset($actividad->imagen) }}" alt="">
                            <div class="card-body d-flex flex-column">
                                ...
                                <ul class="list-group mt-auto">
                                    ...
                                    <li class="list-group-item"><strong>Precio niño:</strong> {{ $actividad->precio_nino }} €</li>
                                    @endif
                            ...
                            <div class="col-12 col-sm-6 col-md-4 d-flex align-items-stretch">
                                <div class="card valoracion-card mx-2 my-2">
                                    <div class="card-body">
                                        ...
                                        <p class="card-text flex-grow-1">{{ $valoracion->comentario }}</p>
                                        <div class="rating">
